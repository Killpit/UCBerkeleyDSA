...
* directions from that cell for a match. We use two arrays x[] and y[] to find next move in all 8 directions*/

/*Time complexity: O(R*C*8*len(str)): All the cells will be visited and traversed in all 8 directions, where R and C is side of
matrix, so time complexity is O(R*C)
*/
//Auxiliary space: O(1) as no extra space is needed

class approach_when_a_single_word_is_given {
    ...
    static boolean search2D(char[][] grid, int row, int col, String word) {
        ...
        for (int dir = 0; dir < 8; dir++) {
            ...
            //If all character matched, then, value of must be equal to the length of word
            if (k == len)
                return true;
        }
        return false;
